...
class Partner(models.Model):
    ...
    def _notify_by_email(self, message, force_send=False, send_after_commit=True, user_signature=True):
        if not message.mapped('subtype_id.template_id'):
            return super(Partner, self)._notify_by_email(message, force_send=force_send, send_after_commit=send_after_commit, user_signature=user_signature)
        message_ids = []
        for me in message:
            if not me.subtype_id.template_id:
                super(Partner, self)._notify_by_email(me, force_send=force_send, send_after_commit=send_after_commit, user_signature=user_signature)
                continue
